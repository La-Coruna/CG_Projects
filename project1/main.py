...
g_key_shift_toggle = False
g_mouse_button_middle_toggle = False
g_cursor_xpos = 0
g_cursor_ypos = 0
...
def cursor_callback(window, xpos, ypos):
    global g_key_shift_toggle, g_mouse_button_middle_toggle, g_cursor_xpos, g_cursor_ypos, g_cam_ang, g_cam_height
    if g_mouse_button_middle_toggle:
        print('mouse_btn_middle: true')
        if xpos < g_cursor_xpos :
            g_cam_ang += np.radians(-1)
            g_cursor_xpos = xpos
        elif xpos > g_cursor_xpos :
            g_cam_ang += np.radians(1)
            g_cursor_xpos = xpos
        if ypos < g_cursor_ypos :
            g_cam_height += -.01
            g_cursor_ypos = ypos
        if ypos > g_cursor_ypos :
            g_cam_height += .01
            g_cursor_ypos = ypos
    elif g_key_shift_toggle:
        if xpos < g_cursor_xpos :
            pass #TODO Camera target change
        elif xpos > g_cursor_xpos :
            pass
...
def button_callback(window, button, action, mod):
    global g_mouse_button_middle_toggle
    if button==GLFW_MOUSE_BUTTON_MIDDLE:
        if action==GLFW_PRESS:
            print('press middle btn: (%d, %d)'%glfwGetCursorPos(window))
            g_mouse_button_middle_toggle = True
            
        elif action==GLFW_RELEASE:
            print('release middle btn: (%d, %d)'%glfwGetCursorPos(window))
            g_mouse_button_middle_toggle = False
